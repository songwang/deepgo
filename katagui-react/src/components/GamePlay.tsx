            ...
            <div style={{ flexShrink: 0 }}>
              ...
              <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'center', alignItems: 'center', width: '480px', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '2px', backgroundColor: '#F5E8C7', borderRadius: '8px', padding: '3px' }}>
                  ...
                  <button onClick={store.nextMove} disabled={!store.hasNextMove} title="Next Move (→)" style={{ background: 'transparent', border: 'none', padding: '4px', cursor: !store.hasNextMove ? 'not-allowed' : 'pointer', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: !store.hasNextMove ? '#BBB199' : '#333333' }}>
                    <FontAwesomeIcon icon={faChevronRight} size="lg" />
                  </button>
                  <button onClick={() => store.goToMove(store.currentPosition + 10)} disabled={!store.hasNextMove} title="Next 10 (Ctrl+→)" style={{ background: 'transparent', border: 'none', padding: '4px', cursor: !store.hasNextMove ? 'not-allowed' : 'pointer', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: !store.hasNextMove ? '#BBB199' : '#333333' }}>
                    <FontAwesomeIcon icon={faForward} size="lg" />
                  </button>
                  <button onClick={store.goToEnd} disabled={!store.canGoToEnd} title="Last Move (End)" style={{ background: 'transparent', border: 'none', padding: '4px', cursor: !store.canGoToEnd ? 'not-allowed' : 'pointer', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: !store.canGoToEnd ? '#BBB199' : '#333333' }}>
