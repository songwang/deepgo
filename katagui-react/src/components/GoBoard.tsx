...
interface GoBoardProps {
  ...
  stones: Map<string, StoneType>; // key: "row,col", value: stone type
  marks?: BoardMark[];
...
const GoBoard: React.FC<GoBoardProps> = ({
  ...
  const pointToKey = (point: Point): string => `${point.row},${point.col}`;
    ...
    stones.forEach((stoneType, key) => {
      ...
      if (stoneType === 'black') {
        const gradient = ctx.createRadialGradient(
          offset + x - stoneRadius * 0.3,
          offset + y - stoneRadius * 0.3,
          stoneRadius * 0.2,
          offset + x,
          offset + y,
          stoneRadius
  ...
  return (
    <canvas
      ...
      onClick={handleClick}
      style={{ cursor: 'pointer', border: '2px solid #8B4513' }}
    />
  );
};
