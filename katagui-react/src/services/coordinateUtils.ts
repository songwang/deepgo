 ...
 */
export function sgfToPoint(sgfMove: string, boardSize = 19): Point | null {
 ...
 */
export function pointToSGF(point: Point, boardSize = 19): string {
 ...
 */
export function isValidPoint(point: Point, boardSize = 19): boolean {
 ...
 */
export function getNeighbors(point: Point, boardSize = 19): Point[] {
 ...
 */
export function getAllPoints(boardSize = 19): Point[] {
  const points: Point[] = [];
 ...
 */
export function getHandicapPoints(boardSize: number, handicap: number): Point[] {
  ...
  if (handicap >= 2) points.push(corners[0], corners[3]); // 2 stones
  if (handicap >= 3) points.push(corners[1]); // 3 stones
  if (handicap >= 4) points.push(corners[2]); // 4 stones
  if (handicap >= 5 && boardSize >= 13) points.push(center); // 5 stones (center)
  if (handicap >= 6) points.push(edges[0], edges[1]); // 6 stones
  if (handicap >= 7 && boardSize >= 13) {
    if (handicap === 7) points.push(center);
    else points.push(edges[2]);
  }
  if (handicap >= 8) points.push(edges[3]); // 8 stones
  if (handicap === 9 && boardSize >= 13) points.push(center); // 9 stones
 ...
 */
export function pixelToPoint(x: number, y: number, cellSize: number, boardSize = 19): Point | null {
 ...
 */
export function pointToPixel(point: Point, cellSize: number): { x: number; y: number } {
  return {
